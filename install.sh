...
echo "=============================="
echo "URL: https://$(kubectl get ingress -n argocd argocd-server -o jsonpath='{.spec.rules[0].host}')"
echo "Username: admin"
echo "Password: $(kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath='{.data.password}' | base64 -d)"
echo "=============================="
echo "URL: https://$(kubectl get ingress -n keycloak keycloak-user-ingress -o jsonpath='{.spec.rules[0].host}')"
echo "Username: temp-admin"
echo "Password: $(kubectl -n keycloak get secret keycloak-initial-admin -o jsonpath='{.data.password}' | base64 -d)"